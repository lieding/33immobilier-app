import { TypologyOptionConfig, LocationType, L1AREA_REGIONS, L2AREA_REGION, loadProgramesByRegions, CsvUrlConfig } from '../common/config';
...
export async function doProgrameQuery (obj, setProgrames) {
  ...
  if (!place_id && locationType && department_city) {
    ...
    if (locationType === LocationType.L1_AREA) {
      ...
      regions.push(postal_code.substring(0, 2));
    }
    return regions.length ? loadProgramesByRegions(regions)
      .then(programes => {
        if (isPostalCode)
