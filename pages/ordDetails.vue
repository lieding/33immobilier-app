    ...
    <div>
      ...
      <div class="details">
         ...
         <p class="price">
             ...
             <span class="unit">{{data.unit}}{{$t("message.global.square€")}}</span>
              <router-link :to="{path: '/instrument'}" tag="span" class="comper">{{$t("message.global.Calculator")}}</router-link>
         </p>
         <p class="identifying">
             <span>
      ...
      <div class="estates" v-if="agent">
          ...
          <div class="broker_box">
              ...
              <hr>
              <div class="num">
                  <p class="phone" v-if="agent.brokerTelPhone"><img src="../../assets/image/phone.png" alt="" class="margin">{{agent.brokerTelPhone}}</p>
                  <p><span class="wx" v-if="agent.wxNumber">{{$t("message.global.WXWach")}}:{{agent.wxNumber}}</span><span class="yx" v-if="agent.brokerEmail">{{$t("message.global.mail")}}:{{agent.brokerEmail}}</span></p>
              </div>
              <div class="quiz">
                <div class="drug"> 
                  <van-cell-group>
                ...
                <van-cell-group>
                    ...
                    <van-button type="primary"  @click="OncontactUs" class="btn_us">{{$t("message.global.woyaotiwen")}}</van-button>
              </div>
     ...
     <div class="estates pack">
          ...
          <span class="interpret download" v-else @click="oninterpret">{{$t("message.global.EasyPro")}}翻译法语</span>
      ...
      <div class="estates">
          <span class="consultants">{{$t("message.global.huxingjiegou")}}</span>
           ...
           <div class="delails">
               <div :style="{width:W}"  v-for="(item,index) in ListTop" :key="index">
                    <div class="houseType_delails">
                        <p class="houseType_conter">{{item.room}}</p>
                        <p class="houseType_conter">{{$t("message.global.proportion")}}：<span>{{item.area}}</span></p>
                        <p class="houseType_conter">{{$t("message.global.orientation")}}：<span>{{item.orientation}}</span></p>
                    </div>
               </div>
                <div :style="{width:W}"  v-for="(items,index) in ListBottom" :key="index">
                     <div class="houseType_delails">
                        <p class="houseType_conter">{{items.room}}</p>
                        <p class="houseType_conter">{{$t("message.global.proportion")}}:<span>{{items.area}}</span></p>
                        <p class="houseType_conter">{{$t("message.global.orientation")}}:<span>{{items.orientation}}</span></p>
                    </div>
               </div>
           </div>
      </div>
  ...
  <div class="estates" >
        ...
        <div class="repay">
          <p class="repay_p">{{$t("message.global.onnetincome")}}</p>
            <van-cell-group>
                <van-field v-model="revenus"  label="€" label-width='.15rem' :placeholder='$t("message.global.qingshuru")' type="Number"/>
            </van-cell-group>
             <p class="repay_p">{{$t("message.global.calculation")}}</p>
            <van-cell-group>
                <van-field v-model="apport" label="€" label-width='.15rem' :placeholder='$t("message.global.qingshuru")' type="Number"/>
            </van-cell-group>
            <p class="repay_p">{{$t("message.global.liability")}} {{$t("message.global.Canfill")}}</p>
            <van-cell-group>
                <van-field v-model="rate" label="%" label-width='.2rem' :placeholder='$t("message.global.qingshuru")' type="Number"/>
            </van-cell-group>
        ...
        <div class="btn">
           ...
           <p class="monthly" v-if="M">{{$t("message.global.highest")}}<span class="money">{{M}}€</span></p>
    ...
    <hr class="hr"/>
     ...
     <div class="estates" >
        ...
        <div class="repay">
            ...
            <van-cell-group>
                <van-field v-model="loan"  label="€" label-width=".15rem" :placeholder='$t("message.global.qingshuru")' type="Number" />
            </van-cell-group>
            <p class="repay_p">{{$t("message.global.payment")}}</p>
            <van-cell-group>
                <van-field v-model="downapport"  @input='Oninput'  label="€" label-width=".15rem" style=" vertical-align: middle" :placeholder='$t("message.global.qingshuru")' type="Number"/>
            </van-cell-group>
            <p class="repay_p">{{$t("message.global.loaninterest")}}</p>
            <van-cell-group>
                <van-field v-model="interesrate"  label="%"  label-width=".2rem" :placeholder='$t("message.global.qingshuru")' type="Number"/>
            </van-cell-group>
            ...
            <p class="monthly" v-if="mothey">{{$t("message.global.highest")}}<span class="money">{{mothey}}</span></p>
           <!-- echarts -->
            <div v-if="mothey" class="canvas">
                <div class="echart"></div>
                <div class="echart_p">
                    <p><span class="green"></span>{{$t("message.global.payment")}} € {{A}}</p>
                    <p><span class="blue"></span>{{$t("message.global.Interestamount")}} € {{L}}</p>
                    <p><span class="orgin"></span>{{$t("message.global.loans")}} € {{S}}</p>
                </div>
            </div>
    ...
    <div class="estates">
     ...
     <van-button type="info" size="large" @click="Onord('/ordIssue')">{{$t("message.global.woyaomaifang")}}</van-button>
     <Footer/>
    </div>
