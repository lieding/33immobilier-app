    ...
    <div>
        ...
        <div class="box">
                  ...
                  <div class="room">
                    ...
                    <div class="details" v-for="(item,index) in rentingApartmentStr" :key="index">
                      <div>
                        ...
                        <el-input :placeholder='$t("message.global.qingshuru")' v-model="item.roomArea" type="text" oninput = "value=value.replace(/[^\d]/g,'')">
                     ...
                     <div>
                        ...
                        <el-select v-model="item.direction" :placeholder='$t("message.global.qingxuanze")'>
                                ...
                                <el-option value="东" :label="$t('message.global.Est')"> {{$t("message.global.Est")}}</el-option>
                                <el-option value="南" :label="$t('message.global.Sud')"> {{$t("message.global.Sud")}}</el-option>
                                <el-option value="西" :label="$t('message.global.Ouest')"> {{$t("message.global.Ouest")}}</el-option>
                                <el-option value="北" :label="$t('message.global.Nord')"> {{$t("message.global.Nord")}}</el-option>
                        </el-select>
                     </div>
                     <div>
                        <p>{{$t("message.global.gender")}}</p>
                          <el-select v-model="item.gender" :placeholder='$t("message.global.qingxuanze")'>      
                              ...
                              <el-option  :label='$t("message.global.woman")'  :value='$t("message.global.woman")'></el-option>
                        </el-select>
                     </div>
                     <div>
                        <p>{{$t("message.global.constellation")}}</p>
                          <el-input :placeholder='$t("message.global.qingshuru")' v-model="item.zodiac" >
                        </el-input>
                     </div>
                     <div>
                        <p>{{$t("message.global.price")}}</p>
                        <el-input :placeholder='$t("message.global.qingshuru")' v-model="item.roomPrice" oninput = "value=value.replace(/[^\d]/g,'')">
                            <template slot="append">€</template>
                        </el-input>
                     </div>
                    </div>
                  
                      <el-button type="primary" @click="addrentingApartment">{{$t("message.global.colocataire")}}</el-button>
                </div>
            </div>
            ...
            <div>
                ...
                <input type="text"  :placeholder='$t("message.global.suchas")' id="txtPlaces" v-on:input ="Onchange" />
                ...
                <el-input :placeholder='$t("message.global.qingshuru")' v-model="total" oninput = "value=value.replace(/[^\d]/g,'')">
                    <template slot="append">€</template>
                </el-input>
            ...
            <div v-if="type=='二手房'">
                ...
                <div class="room">
                    ...
                    <div class="details" v-for="(item,index) in drawingList" :key="index">
                            <div>
                                ...
                                <el-input :placeholder='$t("message.global.qingshuru")' v-model="item.area" oninput = "value=value.replace(/[^\d]/g,'')">
                                    <template slot="append">{{$t("message.global.square")}}</template>
                                </el-input>
                            </div>
                            <div>
                                <p>{{$t("message.global.orientation")}}</p>
                                <el-select v-model="item.orientation" :placeholder='$t("message.global.qingxuanze")'>
                                    <!-- <el-option
                                     v-for="item in orientation"
                                     :key="item.value"
                                     :label="item.label"
                ...
                <div class="room">
                    ...
                    <div class="details" v-for="(item,index) in kitchenList" :key="index">
                      ...
                      <el-button type="primary"  @click="addkitchen">{{$t("message.global.kitchenInformation")}}</el-button>
                </div>
                <div class="room">
                    ...
                    <el-button type="primary"  @click="addshower">{{$t("message.global.bathroomInformation")}}</el-button>
                </div>
                <div class="room">
                    ...
                    <div class="details" v-for="(item,index) in transactionList" :key="index">
                            ...
                            <div>
                                <el-input :placeholder='$t("message.global.qingshuru")' v-model="item.money" oninput = "value=value.replace(/[^\d]/g,'')">
                                    <template slot="append">€</template>
                                </el-input>
                            </div>
                            
                        </div>
                        <el-button type="primary"  @click="addtransaction">{{$t("message.global.addtransactionRecord")}}</el-button>
                </div>
                 <div class="room">
                ...
                <div class="room">
                    <p>{{$t("message.global.sellingTime")}}</p> 
                      <el-select v-model="wishSellingTime" :placeholder='$t("message.global.qingxuanze")' class="select">
                        <el-option
...
<script type="text/javascript" src="http://maps.googleapis.com/maps/api/js?libraries=places&key=AIzaSyAajgcsOMI8iloavKcKSKeoomMB7QVQL4w"></script>
<script>
import base from '@/api/base'; // 导入接口域名列表
import Header from '@/components/MIndex/common/head.vue'
import Footer from '@/components/MIndex/common/footer.vue'
import axios from '@/commen/http'; 
...
//   大盒子
 .box{
  padding: 0.13rem 0.13rem .3rem .13rem;
  div{
          padding: 0.05rem 0;
  }
 }
