  ...
  <ul class="new_list"  v-for="(item,index) in promoteList"  :key="index" >
      <li @click="Onclick(item.promoteId)">
        <diV class="flex">
            ...
            <span class="num" v-if="type=='租房'">{{item.rentType?$t("message.global.sublease"):$t("message.global.entireTenancy")}}</span>
            ...
            <div class="text">
                ...
                <p class="sort" v-if="type=='新房'">{{item.promoteMinHall}} - {{item.promoteMaxHall}}{{$t("message.global.P")}} </p>
                ...
                <p class="font" v-if="type=='新房'">
                 ...
                 <p class="font" v-if=" type == '租房' ">
                         <span class="itemize">{{item.promoteSource?$t("message.global.Personal"):$t("message.global.Intermediary")}}</span>
                        <span class="itemize" v-show="item.promoteSchool == '1'">{{$t("message.global.jinxuexiao")}}</span>
                    
                 </p>

                <p class="money" v-if="type=='新房'">{{item.promoteLowPrice}}{{$t("message.global.rise")}} - {{item.promoteMaxPrice}}{{$t("message.global.rise")}}<span></span></p>
                <p class="money"  v-else-if="type == '二手房'" >{{item.promoteTotal}} {{$t("message.global.rise")}}<span style="font-size:.12rem;color:rgba(172,172,172,1);">{{item.promoteUnit}}€/{{$t("message.global.square")}}</span></p>
                 <p class="money"  v-else-if="type == '租房'" >{{item.promoteTotal}} {{$t("message.global.rise")}}</p>
...
.year{
      ...
      border-radius:0px 0px 0px .13rem;
      font-size:0.12rem;
      font-weight:600;
      color:rgba(255,255,255,1);
      line-height:0.17rem;
