      ...
      <van-dialog
        ...
        <div class="box"> 
          ...
          <p v-if="type == '经纪人' " class="title_jjr"> 
           ...
           <div  class="img"  id="qrCode" ref="qrCodeDiv"  v-if="type != '经纪人'">
           </div>
          <!-- 经纪人 -->
          <!-- <div class="center">
          ...
          <div class="center">
               ...
               <p v-if="listList.type && type != '经纪人'" >{{$t("message.global.PropertyType")}}：{{listList.type}}</p>
               <!-- 标题 -->
               ...
               <p v-if="listList.lowprice">{{$t("message.global.priceRange")}} ：{{listList.lowprice}}€ - {{listList.maxPrice}}€</p>
               <!-- 均价 -->
               <p v-if="listList.unit">{{$t("message.global.averagePrice")}} ：{{listList.unit}}{{$t("message.global.square€")}}</p>
               ...
               <p v-if="listList.estateAddress">{{$t("message.global.position")}} ：{{listList.estateAddress}}</p>
               <p v-if="listList.roomsNum">{{$t("message.global.range")}} ：{{listList.roomsNum}}{{$t("message.global.P")}} - {{listList.bedroomNum}}{{$t("message.global.CH")}}</p>
               <!-- 面积 -->
          
               <p v-if="listList.acreage">{{$t("message.global.proportion")}} ：{{listList.acreage}}{{$t("message.global.square")}} </p>
               <p v-if="listList.minHall">{{$t("message.global.range")}} ：{{listList.minHall}} - {{listList.maxHall}}</p>
            
               <!-- <p v-if="listList.expressing">{{$t("message.global.annual")}}：{{listList.year}}</p> -->
               <p v-if="listList.expressing">{{$t("message.global.completionDate")}} ：{{listList.expressing}} </p>
               <p v-if="type != '经纪人'">{{$t("message.global.Housinglink")}} ：{{href}}</p>
               <p v-if="listList.introduceFr" class="FR">{{$t("message.global.introduce")}} ：{{listList.introduceFr}}</p>
               <p v-if="listList.brokerName">{{$t("message.global.contactName")}} ：{{listList.brokerName}}</p>
               <p v-if="listList.brokerEmail">{{$t("message.global.post")}} ：{{listList.brokerEmail}}</p>

               <p v-if="listList.brokerPhone">{{$t("message.global.contactPhone")}} ：{{listList.brokerPhone}}</p>
          </div>
         <div class="btn">
               <p class="tag-read" :data-clipboard-text="list" @click="copy" v-if="type != '经纪人'">{{$t("message.global.fuzhi")}}</p>
...
          // 不支持复制
              this.$toast({ type: 'danger', message: '该浏览器不支持自动复制' });
